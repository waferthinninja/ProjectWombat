...
namespace LeanPool.Scripts
{
    ...
    public class LeanGameObjectPool : MonoBehaviour
    {
        ...
        [SerializeField] private List<Delay> delays;

        // All the currently despawned prefab instances
        [SerializeField] private List<Clone> despawnedClones;
        ...
        [SerializeField] private List<Clone> spawnedClones;
        ...
        public int Spawned
        {
            get { return spawnedClones != null ? spawnedClones.Count : 0; }
        }

        // Returns the amount of despawned clones
        public int Despawned
        {
            get { return despawnedClones != null ? despawnedClones.Count : 0; }
        }
        ...
        public static LeanGameObjectPool FindPoolByPrefab(GameObject prefab)
        {
            for (var i = Instances.Count - 1; i >= 0; i--)
            {
                ...
                if (pool.Prefab == prefab) return pool;
            }
            ...
            for (var i = Instances.Count - 1; i >= 0; i--)
            {
                ...
                if (pool.spawnedClones != null)
                    for (var j = pool.spawnedClones.Count - 1; j >= 0; j--)
                    {
                        ...
                        if (clone.GameObject == gameObject) return pool;
                    }
            }
            ...
            if (Prefab != null)
            {
                ...
                if (despawnedClones != null)
                    while (despawnedClones.Count > 0)
                    {
                        ...
                        if (Warnings)
                            Debug.LogWarning(
                                "This pool contained a null despawned clone, did you accidentally destroy it?", this);
                    }
                ...
                if (Capacity <= 0 || Total < Capacity)
                {
                    ...
                    if (spawnedClones == null) spawnedClones = new List<Clone>();
                ...
                if (Recycle && spawnedClones != null)
                    while (spawnedClones.Count > 0)
                    {
                        ...
                        if (Warnings)
                            Debug.LogWarning(
                                "This pool contained a null spawned clone, did you accidentally destroy it?", this);
                    }
            }
            else
            {
        ...
        {
            for (var i = spawnedClones.Count - 1; i >= 0; i--) DespawnNow(spawnedClones[i].GameObject);
        }
        ...
        public void PreloadOneMore()
        {
            if (Prefab != null)
            {
                ...
                if (despawnedClones == null) despawnedClones = new List<Clone>();
            ...
            if (delays != null)
                for (var i = delays.Count - 1; i >= 0; i--)
                {
                    ...
                    if (delay.GameObject == cloneGameObject)
                    {
                        if (Warnings)
                            Debug.LogWarning(
                                "You're attempting to delay despawn a gameObject that has already been marked for delay despawn",
                                cloneGameObject);
        ...
        private static LeanPoolable GetOrCachePoolable(Clone clone)
        {
            ...
            if (clone.Poolable == null) poolable = clone.Poolable = clone.GameObject.GetComponent<LeanPoolable>();
        ...
        private void InvokeOnSpawn(Clone clone)
        {
            switch (Notification)
            {
                case NotificationType.SendMessage:
                    clone.GameObject.SendMessage("OnSpawn", SendMessageOptions.DontRequireReceiver);
                    break;
                case NotificationType.BroadcastMessage:
                    clone.GameObject.BroadcastMessage("OnSpawn", SendMessageOptions.DontRequireReceiver);
                    break;
                case NotificationType.PoolableEvent:
                    var poolable = GetOrCachePoolable(clone);
                    if (poolable != null && poolable.OnSpawn != null) poolable.OnSpawn.Invoke();
                    break;
            }
        }
        ...
        private void InvokeOnDespawn(Clone clone)
        {
            ...
            {
                case NotificationType.SendMessage:
                    clone.GameObject.SendMessage("OnDespawn", SendMessageOptions.DontRequireReceiver);
                    break;
                case NotificationType.BroadcastMessage:
                    clone.GameObject.BroadcastMessage("OnDespawn", SendMessageOptions.DontRequireReceiver);
                    break;
                case NotificationType.PoolableEvent:
                    var poolable = GetOrCachePoolable(clone);
                    if (poolable != null && poolable.OnDespawn != null) poolable.OnDespawn.Invoke();
                    break;
            }
        }
