...
namespace Vectrosity.Demos.Scripts.Curve
{
    public class DrawCurve : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            line = new VectorLine("Curve", new List<Vector2>(segments + 1), lineTexture, 5.0f, LineType.Continuous,
                Joins.Weld);
        ...
        {
            anchorObject = Instantiate(anchorPoint, cam.ScreenToViewportPoint(controlLine.points2[pointIndex]),
                Quaternion.identity);
            anchorObject.GetComponent<CurvePointControl>().objectNumber = pointIndex++;
            var controlObject = Instantiate(controlPoint, cam.ScreenToViewportPoint(controlLine.points2[pointIndex]),
                Quaternion.identity);
        ...
        public void UpdateLine(int objectNumber, Vector2 pos, GameObject go)
        {
            var oldPos =
                controlLine.points2[
                    objectNumber]; // Get previous position, so we can make the control point move with the anchor point
            controlLine.points2[objectNumber] = pos;
            var curveNumber = objectNumber / 4;
            var curveIndex = curveNumber * 4;
            line.MakeCurve(controlLine.points2[curveIndex], controlLine.points2[curveIndex + 1],
                controlLine.points2[curveIndex + 2], controlLine.points2[curveIndex + 3],
            ...
            if (objectNumber % 2 == 0)
            {
                ...
                go.GetComponent<CurvePointControl>().controlObject.transform.position =
                    cam.ScreenToViewportPoint(controlLine.points2[objectNumber + 1]);
                ...
                if (objectNumber > 0 && objectNumber < controlLine.points2.Count - 2)
                {
                    ...
                    go.GetComponent<CurvePointControl>().controlObject2.transform.position =
                        cam.ScreenToViewportPoint(controlLine.points2[objectNumber + 3]);
                    line.MakeCurve(controlLine.points2[curveIndex + 4], controlLine.points2[curveIndex + 5],
                        controlLine.points2[curveIndex + 6], controlLine.points2[curveIndex + 7],
        ...
        private void OnGUI()
        {
            if (GUI.Button(new Rect(20, 20, 100, 30), "Add Point")) AddPoint();
            ...
            if (listPoints)
            {
                ...
                {
                    GUILayout.Label("Anchor " + idx + ": (" + (int) controlLine.points2[i].x + ", " +
                                    (int) controlLine.points2[i].y + ")");
                    GUILayout.Label("Control " + idx++ + ": (" + (int) controlLine.points2[i + 1].x + ", " +
                                    (int) controlLine.points2[i + 1].y + ")");
                }
            }
        ...
        private void AddPoint()
        {
            ...
            if (controlLine.points2[pointIndex + 2].x > Screen.width ||
                controlLine.points2[pointIndex + 2].y > Screen.height ||
            ...
            var controlPointPos = controlLine.points2[pointIndex - 1] +
                                  (controlLine.points2[pointIndex] - controlLine.points2[pointIndex - 1]) * 2;
            ...
            var controlObject = Instantiate(controlPoint, cam.ScreenToViewportPoint(controlPointPos),
                Quaternion.identity);
            controlObject.GetComponent<CurvePointControl>().objectNumber = pointIndex++;
            ...
            line.MakeCurve(controlLine.points2[pointIndex - 4], controlLine.points2[pointIndex - 3],
                controlLine.points2[pointIndex - 2], controlLine.points2[pointIndex - 1],
        ...
        private void ChangeSegments()
        {
            ...
            for (var i = 0; i < numberOfCurves; i++)
                line.MakeCurve(controlLine.points2[i * 4], controlLine.points2[i * 4 + 1],
                    controlLine.points2[i * 4 + 2], controlLine.points2[i * 4 + 3], segments, (segments + 1) * i);
            line.Draw();
        }
        ...
        private void ChangeResolution()
        {
            ...
            foreach (var obj in controlPointObjects)
                obj.transform.position =
                    cam.ScreenToViewportPoint(controlLine.points2[obj.GetComponent<CurvePointControl>().objectNumber]);
        }
    }
}