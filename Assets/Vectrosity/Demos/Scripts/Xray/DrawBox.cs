...
namespace Vectrosity.Demos.Scripts.Xray
{
    public class DrawBox : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (mouseDown)
                transform.localScale = new Vector3(worldPos.x - transform.position.x, worldPos.y - transform.position.y,
                    1.0f);
        ...
        private void OnGUI()
        {
            GUI.Box(new Rect(20, 20, 320, 38),
                "Draw a box by clicking and dragging with the mouse\nMove the drawn box with the arrow keys");
            ...
            if (boxDrawn && GUI.Button(buttonRect, "Boom!"))
                foreach (var body in rigidbodies)
                    body.AddExplosionForce(explodePower, new Vector3(0.0f, -6.5f, -1.5f), 20.0f, 0.0f,
