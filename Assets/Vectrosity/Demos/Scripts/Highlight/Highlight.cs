...
namespace Vectrosity.Demos.Scripts.Highlight
{
    public class Highlight : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            energyLine = new VectorLine("Energy", new List<Vector2>(pointsInEnergyLine), null, energyLineWidth,
                LineType.Continuous);
            SetEnergyLinePoints();
        }
        ...
        private void Update()
        {
            ...
            if (Input.GetMouseButtonDown(0) && Input.mousePosition.x > 50 && !fading)
            {
                ...
                if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hit))
                {
                    ...
                    spheres[selectIndex].GetComponent<Renderer>().material
                        .EnableKeyword("_EMISSION"); // So changing emission color will work at runtime
                    selectIndex++;
            ...
            for (var i = 0; i < selectIndex; i++)
            {
                ...
                var squareSize = Screen.height * selectionSize /
                                 Camera.main.transform.InverseTransformPoint(spheres[i].transform.position).z;
                var screenPoint = Camera.main.WorldToScreenPoint(spheres[i].transform.position);
                var thisSquare = new Rect(screenPoint.x - squareSize, screenPoint.y - squareSize, squareSize * 2,
                    squareSize * 2);
                ...
                spheres[i].GetComponent<Renderer>().material
                    .SetColor("_EmissionColor", new Color(energyLevel, energyLevel, energyLevel));
            }
        }
        ...
        private void FixedUpdate()
        {
            ...
            energyLine.points2[i] = new Vector2(energyLine.points2[i].x,
                Screen.height * (.1f + Mathf.Sin((float) timer) * .08f * energyLevel));
        }
        ...
        private void ResetSelection(bool instantFade)
        {
            ...
            if (energyLevel > 0.0f) StartCoroutine(FadeColor(instantFade));
            ...
            else
            {
                ...
                for (var t = 0.0f; t < 1.0f; t += Time.deltaTime)
                {
                    for (var i = 0; i < thisIndex; i++)
                        spheres[i].GetComponent<Renderer>().material
                            .SetColor("_EmissionColor", Color.Lerp(startColor, Color.black, t));
                    yield return null;
                }
        ...
        private void OnGUI()
        {
            GUI.Label(new Rect(60, 20, 600, 40),
                "Click to select sphere, shift-click to select multiple spheres\nThen change energy level slider and click Go");
            ...
            if (selectIndex == 0) energyLevel = 0.0f;
            if (GUI.Button(new Rect(20, Screen.height - 40, 32, 20), "Go"))
            {
                for (var i = 0; i < selectIndex; i++)
                    spheres[i].GetComponent<Rigidbody>().AddRelativeForce(Vector3.forward * force * energyLevel,
